...
exports.generateInspectionPDF = generateInspectionPDF;
...
function resolveImagePath(filePath) {
    if (!filePath)
        return null;
    if (path_1.default.isAbsolute(filePath) && fs_1.default.existsSync(filePath))
        return filePath;
    const rootDir = path_1.default.resolve(__dirname, '../../');
    const candidates = [
        path_1.default.join(rootDir, filePath),
        path_1.default.join(rootDir, filePath.replace(/^\/?uploads\/?/, 'uploads/')),
        path_1.default.join(config_1.UPLOAD_DIR, filePath),
        path_1.default.join(config_1.UPLOAD_DIR, path_1.default.basename(filePath)),
        path_1.default.join(config_1.UPLOAD_DIR, 'demo', path_1.default.basename(filePath)),
...
async function generateInspectionPDF(inspectionId, generatedByUserId) {
    ...
    const container = await database_1.db.get('SELECT * FROM container_details WHERE inspection_id = ?', [inspectionId]);
    const cargo = await database_1.db.get('SELECT * FROM cargo_details WHERE inspection_id = ?', [inspectionId]);
    const loading = await database_1.db.get('SELECT * FROM loading_process WHERE inspection_id = ?', [inspectionId]);
    const product = await database_1.db.get('SELECT * FROM product_details WHERE inspection_id = ?', [inspectionId]);
    const vehicle = await database_1.db.get('SELECT * FROM vehicle_details WHERE inspection_id = ?', [inspectionId]);
    const checklistAnswers = await database_1.db.all(`SELECT a.*, q.question_text, cat.name as category_name
     ...
     ORDER BY cat.order_index, q.order_index`, [inspectionId]);
    const findings = await database_1.db.all('SELECT * FROM findings WHERE inspection_id = ? ORDER BY severity DESC, created_at ASC', [inspectionId]);
    const evidences = await database_1.db.all('SELECT * FROM evidences WHERE inspection_id = ? ORDER BY section, is_primary DESC, created_at ASC', [inspectionId]);
    const signatures = await database_1.db.all('SELECT * FROM signatures WHERE inspection_id = ?', [inspectionId]);
    // Prepare reports directory
    const reportsDir = path_1.default.join(config_1.UPLOAD_DIR, 'reports');
    ...
    const pdfFullPath = path_1.default.join(reportsDir, filename);
    ...
    return new Promise((resolve, reject) => {
        ...
        const writeStream = fs_1.default.createWriteStream(pdfFullPath);
        ...
        const drawKeyValueRow = (label1, val1, label2, val2, isEven) => {
        ...
        if (checklistAnswers && checklistAnswers.length > 0) {
            checklistAnswers.forEach((ans, idx) => {
        ...
        if (photoEvidences.length > 0) {
            ...
            let col = 0;
            let startRowY = doc.y;
            photoEvidences.slice(0, 4).forEach((photo, pIndex) => {
        ...
        const opSig = signatures.find((s) => s.signer_type === 'OPERATOR');
        ...
        const conSig = signatures.find((s) => s.signer_type === 'CONSULTANT');
        ...
        writeStream.on('finish', async () => {
            try {
                ...
                await database_1.db.run(`INSERT INTO reports (id, inspection_id, report_code, pdf_path, version, generated_by, generated_at)
           VALUES (?, ?, ?, ?, 1, ?, datetime('now'))
           ON CONFLICT(report_code) DO UPDATE SET
             pdf_path = excluded.pdf_path,
             version = version + 1,
             generated_by = excluded.generated_by,
             generated_at = datetime('now')`, [reportId, inspectionId, reportCode, pdfRelativePath, generatedByUserId || null]);
